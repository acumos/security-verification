...
public class SecurityVerificationJsonParserTest {
	...
	public void setUp() throws Exception {
		InputStream cdumpInputStream = SecurityVerificationJsonParserTest.class
				.getResourceAsStream("/ACUMOS-CDUMP-BF0478BC-0D3F-4433-80B6-D7A0F6DB2DF1-1.json");
		cdumpInputStreamResult = new BufferedReader(new InputStreamReader(cdumpInputStream)).lines().parallel()
				.collect(Collectors.joining("\n"));
		InputStream siteConfigInputStream = SecurityVerificationJsonParserTest.class.getResourceAsStream("/siteConfig.json");
		siteConfigResult = new BufferedReader(new InputStreamReader(siteConfigInputStream)).lines().parallel()
				.collect(Collectors.joining("\n"));
	...
	public void testParseCdumpJsonFile() throws Exception{
			SecurityVerificationCdump securityVerificationCdump = securityVerificationJsonParser
					.parseCdumpJsonFile(cdumpInputStreamResult);
	...
	public void testExternalScanValue() throws Exception{
			...
			JSONObject siteConfigVerificationObject = (JSONObject) siteConfigDataJsonObj.get(SVConstants.VERIFICATION);
			String externalScanObject = securityVerificationJsonParser.externalScanValue(siteConfigVerificationObject);
	...
	public void testAllowedLicenseMap() throws Exception {
			...
			siteConfigDataJsonObj = securityVerificationJsonParser.stringToJsonObject(siteConfigResult);
			JSONObject siteConfigVerificationObject = (JSONObject) siteConfigDataJsonObj.get(SVConstants.VERIFICATION);
			Map<String, String> allowedLicenseObjectMap = securityVerificationJsonParser
					.allowedLicenseMap(siteConfigVerificationObject);
	...
	public void testSiteConfigMap() throws Exception{
			...
			siteConfigDataJsonObj = securityVerificationJsonParser.stringToJsonObject(siteConfigResult);
			JSONObject siteConfigVerificationObject = (JSONObject) siteConfigDataJsonObj.get(SVConstants.VERIFICATION);
			Map<String, String> licenseScan = securityVerificationJsonParser.siteConfigMap(siteConfigDataJsonObj,
					SVConstants.LICENSESCAN);
			Map<String, String> securityScan = securityVerificationJsonParser.siteConfigMap(siteConfigDataJsonObj,
					SVConstants.SECURITYSCAN);
			Map<String, String> licenseVerify = securityVerificationJsonParser.siteConfigMap(siteConfigDataJsonObj,
					SVConstants.LICENSEVERIFY);
			Map<String, String> securityVerify = securityVerificationJsonParser.siteConfigMap(siteConfigDataJsonObj,
					SVConstants.SECURITYVERIFY);
	...
	public void testScanResultRootLicenseType() throws Exception{
			...
			SecurityVerificationJsonParser securityVerificationJsonParser = new SecurityVerificationJsonParser();
