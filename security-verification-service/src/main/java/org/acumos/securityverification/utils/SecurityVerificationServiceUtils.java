...
public class SecurityVerificationServiceUtils {
  ...
  public static byte[] executeScript(
      String scriptFile, String solutionId, String revisionId, String folder, Environment env)
      throws Exception {
    logger.debug(
        "Inside executeScript scriptFile:{} solutionId:{} revisionId:{}",
        scriptFile,
        solutionId,
        revisionId);

    String scriptFileName =
        SVServiceConstants.FORWARD_SLASH
            + SVServiceConstants.MAVEN
            + SVServiceConstants.FORWARD_SLASH
            + SVServiceConstants.SECURITY_SCAN
            + SVServiceConstants.FORWARD_SLASH
            + scriptFile;
    String folderName =
        SVServiceConstants.FORWARD_SLASH
    ...
    try {
      ...
      if (processBuilder != null) {
        ...
        logger.debug(
            "Cmd1 mkdir folder, Echo command executed, any errors? "
                + (errCode == 0 ? "No" : "Yes"));
        String line = null;
      ...
      if (scriptFile.equalsIgnoreCase(SVServiceConstants.SCRIPTFILE_DUMP_MODEL)) {
        ...
        if (processBuilder != null) {
          ...
          int errCode = process.waitFor();
          logger.debug(
              "dump_model_cmd script, Echo command executed, any errors? "
                  + (errCode == 0 ? "No" : "Yes"));
          String line = null;
          reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
          while ((line = reader.readLine()) != null) {
